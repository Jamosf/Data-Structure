...
// dp[x]的含义是：i表示的二进制对应的索引全部选中的情况下，最小的组合数
func minSessions(tasks []int, sessionTime int) int {
	...
	// 一个111的子集可以由110 + 1构成。即i = 111， y = 110， x^y = 1。x^y，有等于i-j的语义。
	// 另外还有一个更新j的技巧：y = (y-1) &x， y: 110->101->100->011->010->001。这样会出现一个重复计算的问题，因为j减少的时候，会等于i^y。
	// 原则是，j与i^j一定是i的子集。
	for i := 1; i < m; i++ {
		if dp[i] == 1 {
			continue
